...
class Game
{
  ...
  public Game(String c, Date d) throws InvalidCoordinatesException, InvalidArgumentException
  {
    ships = new Boat[5];
    nbrHits = 0;
    nbrShots = 0;
  ...
  */
  ...
  /*
  * Return *-1 if the coord was not fired at
  *        *0 if it landed in the water
  *        *the type of the ship if it was hit
  */
  public int checkStateOfTile(Coord c)
  {
    for(int i = 0; i < shotsFired.size(); i++)
    {
      if(shotsFired.get(i).equals(c))
      {
        return typeOfShipHit.get(i);
      }
    }
    return -1;//Not shot yet
  }

  /*
  * Return -true if the corresponding ship is alive
  *        -false otherwise
  */
  public boolean isShipAlive(int type)
  {
    int index = type-1;
    for(int i = 0; i < ships[index].position().size(); i++)
    {
      if(!IsIn(ships[index].position().get(i),shotsFired))
      {
        return true;
      }
    }
  ...
  /*
  * Return the number of ship that are still alive
  */
  public int nbrShipAlive()
  {
    int nbr = 0;
    for(int i = 1; i <= 5; i++)
    {
